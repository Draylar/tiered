            ...
            if (!hasNbt() || !getNbt().contains("AttributeModifiers", 9))
            {
                ...
                if (potentialAttribute != null)
                {
                    potentialAttribute.getAttributes().forEach(template -> {
                        ...
                        {
                            List<EquipmentSlot> requiredEquipmentSlots = new ArrayList<>(
                                    Arrays.asList(template.getRequiredEquipmentSlots()));
                        ...
                        {
                            List<EquipmentSlot> optionalEquipmentSlots = new ArrayList<>(
                                    Arrays.asList(template.getOptionalEquipmentSlots()));

                            // optional equipment slots are valid ONLY IF the equipment slot is valid for the thing
                            if (optionalEquipmentSlots.contains(slot) && Tiered.isPreferredEquipmentSlot(
                                    (ItemStack) (Object) this, slot))
                            {
                                template.realize(newMap, slot);
                            }
                        }
